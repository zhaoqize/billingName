...
let accountAdd = React.createClass({
  ...
  render() {
    return (
      <div>
        <div className='h2bgc'>
          <h3 style={{ margin:'15px 0 20px 20px',display:'inline-block' }}>渠道 > 账号管理 > 新增账号</h3>
        </div>
        <table>
          <tbody>
          <tr style={{ height:40 }}>
          ...
          <tr style={{ height:40 }}>
            <td className='trodd' ><span className='ant-form-item-required'>确认密码:</span></td>
            <td><Input className='treven' type="text" placeholder="请确认密码"/></td>
          </tr>
          <tr style={{ height:40 }}>
        ...
        <div>
          <Button type="primary" onClick={ this.handleSubmit } style={{ marginLeft:155 }}>提交</Button>
          <Button type="primary" onClick={ this.handleCancel } style={{ marginLeft:60 }}>取消</Button>
        </div>
      </div>
