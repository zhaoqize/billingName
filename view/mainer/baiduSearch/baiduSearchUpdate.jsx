...
import { Select, Form, Input , Button, Icon, DatePicker,Row, Col} from 'antd';
import reqwest from 'reqwest';
import {Link, hashHistory} from 'react-router';

import {ModalSuccess,ModalError} from '../../component/index';
...
var BillingNameInfo = React.createClass({
	...
	componentDidMount:function(){
		const  _this = this;
		this.fetch(rootURL + '/business/BaiduSearch/guess-next-parent',function(result){
...
var AddChannel = React.createClass({
    ...
    componentDidMount:function(){
		...
		//[修改]需拉取计费名下已经关联的数据 http://bnm.stnts.dev/business/BaiduSearch/6/bind-records
		if(this.props.localState.type === 'update'){
	    ...
	    for(i = 0;i < stateconnectdata.length;i ++){
    		...
    		genTr.push(
    			<tr key={ i+1 } style={{ borderBottom: '1px solid #E9E9E9' }}>
					<td>{i+1}</td>
					<td>
						<Select showSearch optionFilterProp="children" notFoundContent="无法找到" style={{ width: 130 }}  {...getFieldProps('channel_channel_' + i,{ initialValue: this.state.connectdata[i].channel + '' })}  >
							{ channel }
						</Select></td>
					<td>
...
var BillingNameUpdate = React.createClass({
	...
	store:function(e){
		...
		var url = _this.props.location.state.type === 'add'? '/business/BaiduSearch/store' : '/business/BaiduSearch/update/' + _this.props.location.state.id ;
