...
var FilterForm = React.createClass({
  ...
  componentDidMount() {
    ...
    //拉子计费名数据
    _this.fetch(rootURL + '/business/BaiduSearch/parents',function(data){
      if(_this.isMounted()){
         _this.setState({
            name:data
          })
...
var TableContent = React.createClass({
  getInitialState() {
    return { 
      url:rootURL + '/business/BaiduSearch/billing-name/' + this.props.locationQuery.state.parentId + '/index' ,//获取parentId=1的子计费名
  ...
  fromFilter:function(params){
      ...
      var url = rootURL + '/business/BaiduSearch/billing-name/' + params.name + '/index';
  ...
  delete:function(channelId){
    return () =>{
      ...
      const _this = this;
       ...
       var del = function(){
            _this.fetch(rootURL + '/business/BaiduSearch/destroy/' + channelId,function(result){
          ...
          render: function(text, record, index){
            ...
            return (
                <div>
                  ...
                  <Link to={{  pathname:"business/BaiduSearch/history",state:{ type:1,id:record.id,name:parentname,child_name:record.name } }} style={{ textDecoration:'underline',marginRight:'5%' }} > 关联历史 </Link>
