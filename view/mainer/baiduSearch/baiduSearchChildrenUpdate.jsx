...
var BillingNameInfo = React.createClass({
  ...
  componentDidMount:function(){
    const  _this = this;
    this.fetch(rootURL + '/business/BaiduSearch/guess-next-child/' + this.props.localState.parentId,function(result){
  ...
  render:function () {
    const { getFieldProps, getFieldError, isFieldValidating } = this.props.localForm;
            ...
            <Col lg={{ span:9 }} sm={{ span:16 }}>
              <Form horizontal  style={{ margin:'10px 0 20px 20px' }} form={this.props.localForm}>
                    <FormItem label="所属广告类型" {...formItemLayout} >
                      ...
                      <Input type="text" style={{ display:'none' }} value='BaiduSearch' {...getFieldProps('info_adtype',{ initialValue:'BaiduSearch' })}/>
                    </FormItem>
                    <FormItem label="父计费名"  {...formItemLayout}>
                      { this.props.localState.parent }
                      <Input type='text' value={ this.props.localState.parent } style={{ display:'none' }} {...getFieldProps('info_parent',{ initialValue:this.props.localState.parentId })}/>
                    </FormItem>
    ...
    if(this.props.localState.type === "update"){
      return (
        <div>
          ...
          <Row>
            <Col lg={{ span:9 }} sm={{ span:16 }}>
              <Form horizontal  style={{ margin:'10px 0 20px -35px' }}>
                    <FormItem label="所属广告类型" {...formItemLayout} >
                      { this.props.localState.adtype }

                      <Input type="text" style={{ display:'none' }} {...getFieldProps('info_adtype',{ initialValue:'BaiduSearch' })}/>
                    </FormItem>
                    <FormItem label="父计费名" {...formItemLayout}>
                      { this.props.localState.parent }
...
var PlatForm = React.createClass({
  ...
  add() {
    ...
    //[修改]需拉取计费名下已经关联的平台 http://bnm.stnts.dev/business/BaiduSearch/6/line-records
    if(_this.props.localState.type === 'update'){
      _this.fetch(rootURL + '/business/BaiduSearch/' + _this.props.localState.id + '/line-records','get',function(result){
...
var AddChannel = React.createClass({
  ...
  add() {
    ...
    //[修改]需拉取计费名下已经关联的数据 http://bnm.stnts.dev/business/BaiduSearch/6/relations
    if(this.props.localState.type === 'update'){
      this.fetch(rootURL + '/business/BaiduSearch/' + this.props.localState.id + '/relations','get',function(result){
        ...
        genTr.push(
          ...
          <td>
            <Select showSearch
                    ...
                    style={{ width: 130 }}  {...getFieldProps('channel_channel_' + i,{ initialValue: this.state.connectdata[i].channel+'' })}  >
              { channel }
            </Select></td>
          <td>
          ...
          <td>
            <Input type="textarea"   placeholder="请填写备注"  {...getFieldProps('channel_comment_' + i,{ initialValue: this.state.connectdata[i].comment })} /></td>
          <td>
            <a onClick={ this.remove } data-index={ i } style={{ textDecoration: 'underline' }}>删除</a></td>
        </tr>
      );
...
var BillingNameChildUpdate = React.createClass({
  formatData:function(obj){
    ...
    for(var i = 0; i < info.length; i++){
      ...
      if(info[i].indexOf('parent') > -1){
        allObj.parent = obj[info[i]];
      }
    }
    ...
    if(this.props.location.state.type === 'update'){
      allObj.id = this.props.location.query.id;
    }
  ...
  store:function(e){
    ...
    var url = _this.props.location.state.type === 'add'? '/business/BaiduSearch/store' : '/business/BaiduSearch/update/' + _this.props.location.query.id ;
    ...
    return(
      <div>
        <div style={{ border:'1px solid #E9E9E9',borderWidth:'0px 0px 1px 0px' }}>
          <h3 style={{ margin:'20px 0 20px 20px' }}>业务 > 百度搜索 { this.props.location.state.type === "add"?"> 新增计费名":"> 修改计费名" }</h3>
        </div>
...
BillingNameChildUpdate = Form.create()(BillingNameChildUpdate)
