	...
	render:function () {
		...
		if(this.props.localState.type === "update"){
			return (
				<div>
					<h2>计费名信息</h2>
					<Form horizontal  onSubmit={this.handleSubmit} style={{ marginLeft:1+'%',marginBottom:1+'%' }}>
				        <FormItem label="所属广告类型" {...formItemLayout}>
				          { this.props.localState.adtype }
				        </FormItem>
				        <FormItem label="投放平台" {...formItemLayout} >
...
var AddChannel = React.createClass({
    ...
    fetch:function(){
		...
		//获取广告位数据
    	reqwest({
			url:'http://localhost:90/business/BaiduXML/ad',
			method: 'get',
      		type: 'json'
		}).then(result => {
			this.setState({
		...
		if(this.props.localState.type === 'update'){
			reqwest({
				url:'http://localhost:90/business/BaiduXML/connectChannel',
				method: 'get',
	      		type: 'json'
			}).then(result => {
				this.setState({
    ...
    submitData:function(){
    	...
    	for(var i=0;i<$channelRangeSel.length;i++){ 
    		...
    		if(flagtwo && $channelRangeSel.eq(i).text() !== ''){
    			channels.channel.push($channelRangeSel.eq(i).text());
    			flagtwo = false;
    		}else if(!flagtwo && $channelRangeSel.eq(i).text() !== ''){
    			channels.ad.push($channelRangeSel.eq(i).text());
    			flagtwo = true;
    		}
    	}
    	...
    	for(var i=0;i<$channelRangeCal.length;i++){ 
	...
	render:function(){
	    ...
	    for (let i = 0; i < this.state.ad.length; i++) {
	        ad.push(<Option key={ this.state.ad[i].label }>{this.state.ad[i].label}</Option>);
	    }

	    //生产类似表格
	    console.log("remove后的connectdata=",this.state.connectdata)
	    ...
	    for(var i=0;i<stateconnectdata.length;i++){
    		...
    		genTr.push(
    			<tr key={ i+1 } style={{ borderBottom: '1px solid #E9E9E9' }}>
					...
					<td>
						<Select style={{ width: 100 }} defaultValue={ stateconnectdata[i].ad }>{ ad }</Select></td>
					<td>
