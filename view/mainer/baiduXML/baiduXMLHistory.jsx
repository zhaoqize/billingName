...
var Header = React.createClass({
	...
	render:function(){
		return (
			<div>
				<h3 style={{ margin:'15px 0 20px 20px',display:'inline-block' }}>业务 > 百度XML > 计费名关联历史( {this.props.localState.name} {this.props.localState.child_name? ' / ' + this.props.localState.child_name:''} )  </h3> 
			</div>
		)
	}
})
...
var AllHistory = React.createClass({
	getInitialState() {
	    return {
	        url : rootURL + '/business/BaiduXML/' + this.props.localState.id + '/relations',
	        data:[],
	        pagination: { //分页对象
