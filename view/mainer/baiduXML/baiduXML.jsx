...
var FilterForm = React.createClass({
  ...
  componentDidMount() {
    ...
    //拉计费名数据
    _this.fetch(rootURL + '/business/BaiduXML/parents',function(result){
        _this.setState({
         name:result.data
        })
    },function(){
      ...
      <Form inline onSubmit={this.handleSubmit} style={{ margin:'20px 0 20px 20px' }}>
        ...
        <FormItem label="当前渠道">
          <Select showSearch 
                  ...
                  notFoundContent="无法找到" 
                  style={{ width: 150 }} {...getFieldProps('channel')} placeholder="请选择渠道" >
           { channel }
          </Select>
        </FormItem>
...
var TableContent = React.createClass({
  getInitialState() {
    return { 
      url: rootURL + '/business/BaiduXML/index',
  ...
  delete:function(id, children){
      return () =>{
        ...
        var del = function(){
           _this.fetch(rootURL + '/business/BaiduXML/destroy/'+ id,function(result){
  ...
  render() {
    ...
    var columns = [{
          ...
          render: function(text, record, index){
               ...
               return <Link to={{pathname:'business/BaiduXML/children',
                                 state:{
                                 parentId:record.id,
                                 name:record.name }}} 
          ...
          render: function(text, record, index){
            ...
            return (
                <div>
                  ...
                  <Link to={{  pathname:"business/BaiduXML/history",state:{ type:0,id:record.id,name:record.name } }} style={{ textDecoration:'underline',marginRight:'5%'}} > 关联历史 </Link>
